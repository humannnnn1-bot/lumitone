...
WCAG 2.2 relative luminance は sRGB を線形化し、`0.2126, 0.7152, 0.0722` を用いる。CSS Color 4 の Candidate Recommendation Draft では Lab/LCH/Oklab/OkLCh などの device-independent / perceptual-oriented color spaces が仕様化されており、HSL の lightness が視覚的明るさと一致しない例として blue/yellow が示されている。
...
| Scope card | Partial | Theory tab includes a Scope and Limits summary at the end; the proposed near-top scope card with explicit BT.601/WCAG/XOR/AND disclaimers is not implemented. |
...
| `theory_dice_desc3` | verified uniqueness claim | keep; cube-net enumeration and hue-path uniqueness are machine-checked |
...
1. `BT.601` coefficients force `GRB` as the unique strictly monotone bit assignment. Implemented in `src/data/__tests__/theory-data.test.ts`.
2. `BT.709/WCAG` coefficient order also satisfies the same `GRB` monotonicity condition; a coefficient-comparison test or UI panel remains to be added.
3. Equal weights create ties and therefore fail strict monotonicity.
4. Complement luma theorem holds for any coefficient triple summing to 1. Implemented for the BT.601 chromatic ranks in `src/data/__tests__/theory-data.test.ts`.
5. Die rank theorem follows from complement order reversal. Implemented in `src/data/__tests__/theory-data.test.ts`.
