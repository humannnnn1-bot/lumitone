...
export const ja = {
  ...
  theory_binary_desc:
    "ここでは、RGB 各チャンネルが 0 または 255 の 8 頂点だけを扱い、各色を3ビット整数 4x₂ + 2x₁ + x₀ として番号付けし、レベル 0..7 を定義します。\n\nRGB の3チャンネルを x₂, x₁, x₀ に割り当てる方法は6通りあります。このうち、番号順と ITU-R BT.601 luma 順が一致する割当は、bit2=Green, bit1=Red, bit0=Blue、つまり GRB だけです。理由は、上位ビットの重みが下位2ビットの重みの和を超え、かつ中位ビットの重みが下位ビットの重みを超えるためです。これは、この有限な RGB 頂点モデルにおける BT.601 luma 係数の不等式（0.587 > 0.299 + 0.114, 0.299 > 0.114）から従う帰結です。\n\nこの割当の下では B=1, R=2, G=4 となり、原色番号は {1, 2, 4} になります。この集合は、元・ペア和・全体和（いずれも通常の整数加算）が重複なく {1, …, 7} を埋め尽くす唯一の3元集合であり、この8色モデルにおける最小の2進基底そのものです。",
  theory_binary_color: "色",
  theory_binary_luma_formula: "Luma (BT.601): Y′ = 0.299R + 0.587G + 0.114B",
  theory_binary_luma_complement: "補色ルマ定理: Y′\u2096 + Y′\u2087\u208b\u2096 = 255（係数の和が1であれば任意の係数で成立）",
